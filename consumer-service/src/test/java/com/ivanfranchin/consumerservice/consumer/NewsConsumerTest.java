    ...
    try (ConfigurableApplicationContext context =
        new SpringApplicationBuilder(
            ...
            .run(
                ...
                "--spring.cloud.stream.bindings.news-in-0.destination=ps/news/*/BR>")) {
      ...
      assertThat(output)
          .contains("PAYLOAD: News[id=id, type=type, country=country, city=city, title=title]");
    }
  }
  ...
  void testNewsSubscribingAllEconomyNews(CapturedOutput output) {
    try (ConfigurableApplicationContext context =
        new SpringApplicationBuilder(
            ...
            .run(
                "--spring.jmx.enabled=false",
                "--spring.cloud.stream.bindings.news-in-0.destination=ps/news/ECONOMY/>")) {
  ...
  void testNewsSubscribingAllNewsFromPorto(CapturedOutput output) {
    try (ConfigurableApplicationContext context =
        new SpringApplicationBuilder(
            ...
            .run(
                "--spring.jmx.enabled=false",
                "--spring.cloud.stream.bindings.news-in-0.destination=ps/news/*/*/Porto")) {
